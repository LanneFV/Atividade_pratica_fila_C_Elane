...
typedef struct {                                    //Define a struct para armazenar os dados do paciente.
    int id;                                         //ID do paciente. -->IDENTIFICAÇÃO DO PACIENTE<--
    char nome[50];                                  
} paciente;

typedef struct elemento_fila {                      //Define a struct para os elementos da fila.
    paciente dados;                                 
    struct elemento_fila* prox;                          
} Elemento;

typedef struct fila{                                //Define a struct para a fila em si. -->CONTROLE DA FILA<--
    Elemento* inicio;
    Elemento* fim;
    int tamanho;
} Fila;
