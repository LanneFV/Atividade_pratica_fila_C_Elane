...
int lista_vazia(Fila* fila) {                           //Passa como parâmetro um ponteiro para a fila. (condicional para saber se a fila está vazia)
...
void liberar_fila(Fila* fila) {                         
    ...
    while (atual != NULL) {                             //Enquanto o ponteiro atual não for NULL, percorrer a fila até que todos os elementos sejam liberados.
...
void enfileirar(Fila* fila, paciente dados) {           //Passa como parâmetro um ponteiro para a fila e os dados do paciente a ser enfileirado.
    Elemento* novo_elemento = (Elemento*) malloc(sizeof(Elemento));//Aloca memória para um novo elemento da fila.
    if(novo_elemento == NULL){                          //Verifica se a alocação foi bem sucedida.
    ...
    if (fila->fim != NULL) {                            //Se o fim da fila não for NULL, ou seja, se a fila não estiver vazia.
    ...
    if (fila->inicio == NULL) {                         //Se o início da fila for NULL, ou seja, se a fila estiver vazia.
...
paciente desenfileirar(Fila* fila) {                                                              
    if (lista_vazia(fila)) {                            //Verifica se a fila está vazia.
    ...
    if (fila->inicio == NULL) {                         //Se o início da fila for NULL após a remoção, ou seja, se a fila ficar vazia.
...
int tamanho_fila(Fila* fila) {                          //Passa como parâmetro um ponteiro para a fila. -->SERVE PARA SABER QUANTOS PACIENTES TEM NA FILA<--
    return fila->tamanho;                               //Retorna o tamanho da fila.
}

//FUNÇÃO PARA IMPRIMIR OS DADOS DE UM PACIENTE
void print_paciente(paciente paciente) {                //Passa como parâmetro uma struct paciente. -->SERVE PARA PASSAR AS INFORMAÇÕES DO PACIENTE<--
...
void imprimir_fila(Fila* fila) {
    Elemento* atual = fila->inicio;                     //Cria um ponteiro para percorrer a fila, começando pelo início.
    while (atual != NULL) {                             //Enquanto o ponteiro atual não for NULL, percorre a fila.
